...
export default function Home() {
  ...
  useEffect(() => {
    const hasGalleryClass =
      document.documentElement.classList.contains("work-layout") ||
    ...
    const hasGalleryVars =
      getComputedStyle(document.documentElement).getPropertyValue(
        "--c-white",
      ) ||
      getComputedStyle(document.documentElement).getPropertyValue("--c-black");

    if (isFromGallery || hasGalleryVars || hasGalleryClass || visitedGallery) {
  ...
  return (
    ...
    >
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 items-center justify-center gap-8 mt-8">
        <Link
          href="/gallery"
          className="bg-neutral-800 w-40 h-32 rounded-xl text-lg flex items-center justify-center"
        >
          gallery
        </Link>
        <Link
      ...
      <Link
        href="https://github.com/abdout/camille"
